...
## 13.2.2 스레드 안전성 구현
- 스레드 안전성을 구현하는 코드 작성법
- 가상 머신이 제공하는 동기화와 락 메커니즘
...
- java.util.concurrent 패키지 (Concurrency utilities)
...
- 둘 이상의 조건 지정
    ...
    - synchronized 도 락 객체의 wait(), notify(), notifyAll() 메서드는 모두 특정 조건을 구현할 수 있다. 하지만, 둘 이상 조건을 주고 싶다면 또 락을 추가해야 한다.
